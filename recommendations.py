def generate_recommendations(total_income, total_spent, balance, category_spending):
    ...
    if total_income > 0:
        ...
        if savings_rate < 20:
            recommendations.append(
                "Your savings rate is below 20%. Reduce non-essential spending."
            )
        else:
            recommendations.append(
                "Your savings rate is healthy. Maintain your current spending control."
            )
    else:
        recommendations.append(
            "Income data is missing for this month, so savings rate cannot be calculated."
        )
    ...
    if total_spent > total_income:
        recommendations.append(
            "Your expenses are higher than your income. Immediate budget correction is needed."
        )
